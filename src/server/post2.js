...
module.exports = class Post {
  ...
  async save() {
    this.data.id = await dbConnection.getNewID();


    this.data.totalCurrentAddresses = (
      await dbConnection.getAllKeys(InMemoryDB.ADDRESSES_DB)
    ).length;
  ...
  static async all(filters = {}) {
    let posts = await dbConnection.getAllKeys(InMemoryDB.POSTS_DB);

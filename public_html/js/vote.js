    ...
    el.addEventListener("click", (e) => {
      if (!user) {
        ...
        if (membership) {
          ...
          const checkedRadio =
            $('input[name="post-' + obj.id + '-options"]:checked') || null;
            ...
            fetch(url + "vote", {
              ...
              .then((data) => {
                ...
                if (!snd.paused) {
                  ...
                  queryURL.type = "title";
                  queryURL.id = obj.id;
                  queryURL.string = $("#post-" + obj.id + " .title")
          ...
          if (accountStatus.balance >= minBalance) {
            ...
            message += `Hold ${minBalance} BEBE in the account: NO\n`;
          }

          if (accountStatus.kyc === true) {
            message += "Pass the KYC process: OK";
          } else {
            message += "Pass the KYC process: NO";
          }

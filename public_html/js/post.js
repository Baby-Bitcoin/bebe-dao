...
import { attachListenersToCommentBoxes, drawPostCommentsSection, } from "./comments.js";
import { checkFileProperties, handleUploadedFile } from "./image-select.js";
import { currentPostsFilters } from "./search.js";
import { $, $$ } from "./ui.js";
import { countdown, currentUnixTimestamp, formatDate, shorthandAddress, wait, } from "./utilities.js";
import { attachListenersToVote } from "./vote.js";
import { features } from "./welcome.js";
let filter = null;
let globalPost = {};
const handlePostSubmit = () => __awaiter(void 0, void 0, void 0, function* () {
    $("#post-form").addEventListener("submit", (event) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        formData.append("quorum", $("#quorumSlider").value);
        imageElement.files[0] && formData.append("image", imageElement.files[0]);
        $$(".voteInput").forEach((option) => formData.append("options[]", option.value));
        ...
        (votes || []).forEach((vote) => formData.append("votes[]", vote));
        try {
            const post = yield fetch("/post", {
                method: "POST",
                body: formData,
...
const areCommentsAllowed = (post) => {
    return !(post.type == "election" && !isPostClosed(post));
};
const postCountdown = (post) => {
...
const postFormListener = () => {
    ...
    $("#post-form").addEventListener("change", (event) => {
        ...
        if ($('input[name="type"]:checked').value === "poll" ||
            $('input[name="type"]:checked').value === "election") {
            $(".options").style = "";
...
const attachListenersToAddresses = () => {
    $$(".post span[data-address]").forEach((el) => {
        el.addEventListener("click", function (event) {
            ...
            postActions(Object.assign(Object.assign({}, currentPostsFilters()), { address }));
        });
    });
};
const drawPostDetails = ({ post, address, comments, votes, ADMINS }) => {
    var _a, _b, _c;
    let deletePost = "";
    ...
    if ((_b = (_a = votes.voters) === null || _a === void 0 ? void 0 : _a.includes) === null || _b === void 0 ? void 0 : _b.call(_a, publicKey)) {
    ...
    description = description
        ...
        .replace(/<\/script>/g, "</code>");
    const usersNeeded = Math.ceil((post.quorum / 100) * post.totalCurrentAddresses);
    const totalMembers = `<b>${((_c = votes.voters) === null || _c === void 0 ? void 0 : _c.length) || 0}</b>/${usersNeeded} users (${post.quorum}%)`;
    const htmlStr = `
...
const attachListenersToPost = (post, address = {}) => {
...
const deletePost = (post) => __awaiter(void 0, void 0, void 0, function* () {
    const promptString = prompt("Are you sure you want to delete this post?", "YES");
    if (promptString != "YES") {
        return;
    }
    const result = yield fetch("/delete-post", {
        method: "DELETE",
        headers: {
...
const drawPost = (post) => {
    ...
    post.voted === false
        ? (voted = false)
        : (voted = (_a = post === null || post === void 0 ? void 0 : post.voted) === null || _a === void 0 ? void 0 : _a.includes(localStorage.getItem("publicKey")));
    if (isClosed) {
...
const postActions = (...args_1) => __awaiter(void 0, [...args_1], void 0, function* (filters = {}) {
    ...
    const posts = yield fetch(`/posts?${params.join("&")}`).then((response) => response.json());
    ...
    if (filters.query || filters.tag || filters.address) {
        $(".query-text").innerHTML = `<b>${filters.query || filters.tag || filters.address}</b> - returned ${posts.length} results.`;
    }
    attachListenersToAddresses();
});
const loadSinglePost = (postId) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const response = yield fetch(`/posts/${postId}`);
        $("#loader").style.setProperty("display", "none");
        if (!response.ok) {
...
export { postActions, filter, loadSinglePost, isPostClosed, areCommentsAllowed, globalPost, drawPostDetails, };
